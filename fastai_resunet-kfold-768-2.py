...
for i in range(5):
    ...
    wd=1e-7
    lr = 1e-3
    lrs = np.array([lr,lr/10,lr/100])
    
    learn.load('best_res34_768_la')
    learn.unfreeze()
    learn.bn_freeze(True)
    
    logname = 'best_res34_768_fold{}_3'.format(i)
    tb_logger = TensorboardLogger(learn.model, md, 'log/' + logname, metrics_names=["IoU"])
    learn.fit(lrs/5, 1, wds=wd, cycle_len=3, use_clr=(10,8), 
              best_save_name='best_res34_768_fold{}_3'.format(i),
              callbacks=[tb_logger])
    
    learn.save('Unet34_768_fold{}_3'.format(i))
